...
export default function SendMessage() {
  ...
  const [suggestError, setSuggestError] = useState<null | string>(null);
  ...
  const fetchSuggestedMessages = async () => {
    ...
    try {
      ...
      const axiosError = error as AxiosError<ApiResponse>;
      setSuggestError(axiosError.message || 'Error fetching suggestions');
    } finally {
            ...
            {suggestError ? (
              ...
              parseStringMessages(suggestions).map((message, index) => (
                <Button
                  key={index}
                  variant="outline"
                  className="mb-2 w-full text-left whitespace-normal break-words h-auto py-3 px-4"
